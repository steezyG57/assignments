...
const welcome = readlineSync.question("Welcome to the Colossal Role-Playing Game! Follow the forbidden path to embark on your journey to your salvation! What is your name?")
...
function fightEnemy (enemy, enemyHp) {
    ...
    while (playerHp >= 0 && enemyHp >= 0) {
        ...
        if (fight === "f") {
            ...
            if(enemyHp <= 0) {
                playerHp = hpReceivedFromKillingEnemy + playerHp
                console.log(`\nThe enemy is dead! You received ${hpReceivedFromKillingEnemy} HP points and ${potionReceivedFromKillingEnemy} potion(s)`)
                inventory.potion = inventory.potion + potionReceivedFromKillingEnemy
                console.log(inventory)
                walk()
            } else if ( playerHp <= 0) {
                console.log(`\n${enemy} has killed you! Your journey is over!\n`)
                distanceLeftToTravel = -1
            }
        } else if (fight === "r") {
            chancOfEscaping = randomGenerator();
            if (chanceOfEscaping >= 50) {
                ...
                console.log("\n You fail to escape!")
                playerHp = playerHp - damageInflictedOnPlayer
                console.log("\nThe enemy attacked you while you tried to flee! Now your health is: " + playerHp)
